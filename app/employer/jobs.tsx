...
import type { JobDetails, JobSalaryType, JobType } from "@/stores/jobStore";

const { width } = Dimensions.get("window");

export default function EmployerJobs() {
	const { user } = useAuthStore();
	const { jobs, isLoading, error, fetchJobs, addJob, addJobViaRPC, deleteJob } =
		useJobStore();
	...
	const handleAddJob = async () => {
		...
		const newJobDetails: JobDetails = {
			...
			slotsAvailable: Number.parseInt(jobSlotsAvailable) || 1,
			tags: jobTags
				.split(",")
				.map((tag) => tag.trim())
				.filter((tag) => tag !== ""),
		...
		if (jobId) {
			Alert.alert(
				"Success",
				`Job posted successfully${useRPC ? " via RPC" : ""}`,
			);
			...
			<View style={styles.jobMeta}>
				<View style={styles.jobTypeBadge}>
					<Text style={styles.jobTypeText}>
						{item.jobType?.replace("_", " ") || "Unknown"}
					</Text>
				</View>
						...
						<ScrollView style={styles.formContainer}>
								...
								<View style={styles.pickerContainer}>
									...
									>
										<Text style={styles.pickerText}>
											{jobSalaryType?.replace("_", " ") || "per hour"}
										</Text>
							...
							<View style={styles.toggleContainer}>
								...
								<Text style={styles.toggleText}>
									{useRPC
										? "Using RPC API call"
										: "Using direct database insertion"}
								</Text>
							</View>
							<TouchableOpacity
